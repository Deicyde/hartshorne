...
Three ingredients are now available — `S_(xᵢ) ≅ k[y]`, the ideal
correspondence, and the quotient grading — and what remains is to assemble them
into `S(Y)_(xᵢ) ≅ A(Yᵢ)`. The step to do that is localisation-quotient exchange
in the graded setting: `S(Y)_(xᵢ) ≅ S_(xᵢ) ⧸ 𝔞`, where `𝔞` is generated by the
degree-zero fractions `g/xᵢⁿ` with `g ∈ J(Y)` homogeneous of degree `n`. Given
that, the ideal correspondence identifies the image of `𝔞` under
`S_(xᵢ) ≅ k[y]` with `I(Yᵢ)`, and (b) and (c) follow from the affine case by
